...
try:
    # Route 1: this user's waste records with FoodName and Category
    r_user = requests.get(f"{BASE_URL}/foodWaste/{user_id}")
    if r_user.status_code == 200:
        user_waste = r_user.json()
    else:
        st.error(f"Error fetching user waste: {safe_error(r_user)}")

    # Route 2: cost summary
    ...
    if r_cost.status_code == 200:
        ...
        st.warning(f"Could not load cost data: {safe_error(r_cost)}")
...
try:
    ...
    if r_activity.status_code == 200:
        ...
        st.warning(f"Could not load activity data: {safe_error(r_activity)}")
...
# ── Chart 2: This user's wasted food by category ──────────────────────────────
with col2:
    st.subheader("Your wasted food by category")

    if not user_waste:
        st.info("No waste data available.")
    else:
        df_waste = pd.DataFrame(user_waste)
        df_waste["Amount"] = pd.to_numeric(df_waste["Amount"], errors="coerce").fillna(0)
...
if cost_data:
    ...
    total_cost = float(cost_data.get("TotalCostWasted") or 0.0)
    st.metric(
